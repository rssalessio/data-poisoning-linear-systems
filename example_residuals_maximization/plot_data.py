...
results: List[ResultsSimulation] = np.load('data/data.npy', allow_pickle=True)
N_SIMS = len(results)
...
for i in range(N_SIMS):
    ...
    for j in range(len(res.deltas)):
        ...
        ABtilde, poisoned_residuals = evaluate_attack(Xm, Xp, res.U, res.unif_DeltaX[j], res.unif_DeltaU[j])
        unif_poisoned_delta_norm[i,j] = np.linalg.norm(ABtilde - res.TrueAB, 2)
        unif_poisoned_delta_test[i,j] = residuals_variance_test(poisoned_residuals, dim_u, sigma_eigs).p_right

        DeltaAB = ABtilde - res.TrueAB
        unif_poisoned_delta_dot[i,j] = angle_between(ABnorm, DeltaAB.flatten())#np.abs(np.dot(ABnorm, DeltaAB.flatten()))
...
impact_unif_mean, impact_unif_lower, impact_unif_upper = confidence_interval_signal(unif_poisoned_delta_norm, N_SIMS)
...
resvar_unif_mean, resvar_unif_lower, resvar_unif_upper = confidence_interval_signal(unif_poisoned_delta_test, N_SIMS)
...
ax[2].plot(deltas, dot_unif_mean, ':', label='Uniform attack')
...
plt.savefig('figures/example_residuals_variance.png',bbox_inches='tight')
